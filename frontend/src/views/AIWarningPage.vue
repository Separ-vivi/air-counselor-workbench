  ...
  <div class="ai-warning-page">
    ...
    <div v-else class="warning-table">
      ...
      >
        ...
        <!-- V6.17: 操作列 - 发起谈心谈话，联动访谈页面 + 已谈话状态标识 -->
        <el-table-column label="状态/操作" width="140" fixed="right" align="center">
          <template #default="{ row }">
            <div v-if="interviewedStudentIds.has(row.student_id)" class="interview-done">
              <el-tag type="success" size="small" effect="plain" round>
                ✓ 已发起
              </el-tag>
              <el-button type="primary" size="small" text @click="startInterview(row)">再次谈话</el-button>
            </div>
            <el-button v-else type="primary" size="small" @click="startInterview(row)">发起谈话</el-button>
          </template>
        </el-table-column>
      </el-table>
...
// V6.17: 已发起谈话的学生ID集合
const interviewedStudentIds = ref(new Set())
...
// V6.17: 加载已发起谈话的学生状态
const loadInterviewStatus = async (studentList) => {
  if (!studentList || !studentList.length) {
    interviewedStudentIds.value = new Set()
    return
  }
  try {
    const ids = studentList.map(w => w.student_id).filter(Boolean).join(',')
    if (!ids) return
    const res = await http.get('/interview/warning-interview-status', { params: { student_ids: ids } })
    interviewedStudentIds.value = new Set(res?.interviewed_student_ids || [])
  } catch (e) {
    interviewedStudentIds.value = new Set()
  }
}
...
const refreshWarnings = async (force = false) => {
  ...
  try {
    ...
    // V6.17: 加载已发起谈话的学生状态
    await loadInterviewStatus(warnings.value)
  } catch (e) {
