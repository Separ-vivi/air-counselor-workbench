...
const stats = computed(() => {
  ...
  const parents = new Set(list.map(r => r.parent_name).filter(Boolean)).size
  const latestDate = list
    .map(r => r.contact_date)
    .filter(Boolean)
    .sort()
    .pop() || '—'
  return [
    { label: '联系记录数', value: total },
    { label: '联系人数', value: parents || '—' },
    { label: '最近联系', value: latestDate }
  ]
})
