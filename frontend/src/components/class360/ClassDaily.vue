...
import { getClassDaily } from '@/api/class360.js'

const props = defineProps({ cid: { type: [String, Number], required: true } })
...
const stats = computed(() => {
  ...
  const disciplines = (r.disciplines || []).length
  const attendance = (r.attendance || []).length
  return [
    { label: '请假记录', value: leaves || '—' },
    { label: '违纪记录', value: disciplines || '—' },
    { label: '考勤异常', value: attendance || '—' }
  ]
})
...
async function load() {
  ...
  loading.value = true
  try {
    raw.value = await getClassDaily(props.cid) || {}
  } catch { raw.value = {} }
