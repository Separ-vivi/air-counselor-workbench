<template>
  <div class="tab-summary">
    <div class="summary-header">
      <h3>💚 心理关注概览</h3>
      <el-button type="primary" @click="$router.push('/module/psychology')">
...
import { getClassPsychology } from '@/api/class360.js'
...
const stats = computed(() => {
  ...
  const warning = list.filter(r => (r.warning_level || r.level || '').includes('警')).length
  const focus = list.filter(r => (r.attention_level || r.focus || '').includes('关')).length
  return [
    { label: '总记录数', value: total || '—' },
    { label: '关注人数', value: focus || '—' },
    { label: '预警人数', value: warning || '—' }
  ]
})
...
async function load() {
  ...
  loading.value = true
  try {
    data.value = await getClassPsychology(props.cid)
