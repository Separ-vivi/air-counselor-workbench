...
import { getClassActivities } from '@/api/class360.js'
...
const stats = computed(() => {
  ...
  const participants = list.reduce((sum, a) => sum + (Number(a.participant_count || a.signup_count || 0)), 0)
  return [
    { label: '本学期活动数', value: total || '—' },
    { label: '参与人次', value: participants || '—' },
    { label: '活动类型数', value: new Set(list.map(a => a.activity_type || a.type).filter(Boolean)).size || '—' }
  ]
})
